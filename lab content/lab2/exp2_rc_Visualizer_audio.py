...
from scipy.signal import remez, filtfilt, resample_poly 
import cv2
import Preliminary
...
class timeScope:
    def __init__(self):
        self.fig, self.ax_spectrum = plt.subplots(1, 1, figsize=(7, 4), sharex=True, num='Time Scope')
        ...
        # Time scope for real part of bandpass filtered signal and demodulated signal 
        self.real_part, = self.ax_spectrum.plot([], [], color='red', linewidth=1.0, label='Filtered')
        self.demod, = self.ax_spectrum.plot([], [], color='blue', linewidth=1.0, label='Demodulated')
    ...
    def addNewData(self, t, real, demod):
        self.ax_spectrum.plot(t, real, color='red', linewidth=1.0)
        self.ax_spectrum.plot(t, demod, color='blue', linewidth=1.0)
...
class Receiver:
    def __init__(self):
        ...
        self.RC_const_multiplier = None
        self.fc = None
        self.gain = None                   # Pluto SDR tuner gain in dB
        self.audio_fs = None               # Sampling rate of audio signal
        self.listen_time = None
    ...
    def loadParameters(self):
        ...
        if self.audio_fs is None:
            self.audio_fs = 48.0e3
        self.demodulated_visualizer.audio_fs = int(self.audio_fs)
    ...
    def receiveData(self):
        ...
        self.real_bpf_sig = np.real(self.bpf_sig)                   # Real part of filtered data

        # Envelope detection
        self.demod_sig  = Preliminary.Pre_Q2_func(self.real_bpf_sig, self.Deltaf, self.sample_rate, self.RC_const_multiplier)

        # Resample to be listened
        tolerance = 1e-12
        rat = Fraction(self.audio_fs / self.sample_rate).limit_denominator(int(1/tolerance))
        L = rat.numerator
        M = rat.denominator
        self.demod_audio = resample_poly(self.demod_sig, L, M)
        self.demod_audio = self.demod_audio / (np.max(np.abs(self.demod_audio)) + 1e-12)
        ...
        self.f_audio, self.Pxx_demod_audio = welch(self.demod_audio, fs=self.audio_fs, window='hann',
                    nperseg=self.fftsize, noverlap=None, 
                    nfft=None, detrend=None, return_onesided=False,
                    scaling='spectrum', axis=0, average='mean')
        ...
        self.f_audio = np.fft.fftshift(self.f_audio)
        self.f_audio_kHz = self.f_audio / 1e3
        ...
        self.Pxx_demod_audio = np.fft.fftshift(self.Pxx_demod_audio)
        self.Pxx_demod_audio_dB = 10 * np.log10(self.Pxx_demod_audio + 1e-12)   # Avoid log of zero with small epsilon
        self.Pxx_demod_audio_dB = self.Pxx_demod_audio_dB + 10 * np.log10(1e3)  # dB to dBm

    def capture(self):
        # Calculate the total number of samples required
        total_samples = int(self.sample_rate * self.listen_time)
        ...
        while samples_captured < total_samples:
            ...
            captured_data.append(samples)

            # Update the count of captured samples
            samples_captured += len(samples)
        captured_data = np.array(captured_data).reshape(-1)
    ...
    def prepare_audio(self):
        ...
        data_to_listen = self.capture()
        bpf_sig = filtfilt(self.bpFilt, 1.0, data_to_listen)     # Filtered signal
        real_bpf_sig = np.real(bpf_sig)
        demod_sig  = Preliminary.Pre_Q2_func(real_bpf_sig, self.Deltaf, self.sample_rate, self.RC_const_multiplier)

        audio = resample_poly(demod_sig, L, M)
        audio = audio / (np.max(np.abs(audio)) + 1e-12)

        print(data_to_listen.shape)
        print(audio.shape)
        return audio
    
    def visualize(self):
        self.simulation_ended = self.modulated_visualizer.visualizeNewData(self.Pxx_dB, self.Pxx_bpf_dB, self.f_kHz)
        self.simulation_ended = self.demodulated_visualizer.visualizeNewData(self.Pxx_demod_audio_dB, self.f_audio_kHz)
    ...
    def timeScope(self):
        ...
        while run_time<simulation_duration and index_counter<len(data_bpf_real):
            ...
            data_demod[index_counter].set_ydata(self.demod_sig * 1e3) 
            
            self.timeScopeVis.ax_spectrum.relim()           # Recalculate limits
            self.timeScopeVis.ax_spectrum.autoscale_view()  # Autoscale based on current data
