...
def calculate_symmetry(psd, f_l_idx, f_h_idx):
    ...
    # Calculate correlation coefficient (close to 1.0 means symmetric/DSB-SC)
...
def burst_analyze(idx):
    ...
    if detection_map[idx]:
        ...
        # New Symmetry Check
        sym_score = calculate_symmetry(psd_smoothed, l_idx_10, r_idx_10)
        
        info = burst_info[idx]
        
        if info['is_lfm'] and env_var < 0.01:
            ...
            # High symmetry = Mirrored sidebands
            mod_type, color = f"DSB-SC (Type 2) | Symmetry: {sym_score:.2f}", 'green'
        else:
            # Low symmetry = Single sideband
            mod_type, color = f"SSB (Type 3) | Symmetry: {sym_score:.2f}", 'lime'
