...
# --- NEW: SYMMETRY CALCULATION FUNCTION ---
def calculate_symmetry(psd, f_l_idx, f_h_idx):
    ...
    # Reverse right half to compare mirrored indices directly
...
# --- NEW: CONSENSUS CLASSIFICATION LOGIC ---
print("Calculating consensus modulation types for bursts...")
burst_info = {}
burst_votes = {i + 1: [] for i in range(len(starts))}

# Pre-classify every buffer to collect votes
for i in range(len(starts)):
    ...
    is_lfm_sweep = total_sweep_khz > 40.0 
    ...
    for idx in range(s_idx, e_idx + 1):
        ...
        psd_smoothed = savgol_filter(10 * np.log10(psd_lin + 1e-12), window_length=51, polyorder=3)
        peak_idx = np.argmax(psd_smoothed)
        ...
        # Individual classification for voting
        if is_lfm_sweep and env_var < 0.01:
            label = "LFM (Type 5)"
        ...
        burst_votes[i + 1].append(label)
        burst_info[idx] = {'id': i + 1, 'duration': dur, 'total_sweep': total_sweep_khz}

# Determine the most common modulation type for each burst ID
burst_consensus = {}
for b_id, votes in burst_votes.items():
    if votes:
        most_common = Counter(votes).most_common(1)[0][0]
        burst_consensus[b_id] = most_common
...
def burst_analyze(idx):
    ...
    # --- TIME DOMAIN & HIGH PRECISION ENVELOPE ---
    ...
    if detection_map[idx]:
        ...
        info = burst_info[idx]
        mod_type = burst_consensus[info['id']] # Consensus label

        # Define color based on consensus label
        color_map = {
            "LFM (Type 5)": "magenta",
            "AM (Type 1)": "orange",
            "FM (Type 4)": "cyan",
        ...
        ax1.set_title(f"Buffer {idx} | CONSENSUS: {mod_type} | Burst #{info['id']}", color=color, fontweight='bold')
    else:
